...
/*      if (!RELEASE_BUILD &&  LOG_TO_FILE) {
          ...
          getLogger().info("File logging active : " + new File(fileTarget.logURI).nativePath);
        }
        else
          getLogger().info("File logging disabled...");  */  
      }    
      
      /*     private static function logEvent(e:Event) : void {    
      logger.info(e.target + ":" + e.type);
      } */
      ...
      {         
        ...
        /* if (versionStr.indexOf("\.")>=0) {
          var parts:Array = versionStr.split("\.");
          var vers:String = parts[0]; 
          var instanceIdStr:String = parts[1];
          instanceId = parseInt(parts[1]);
          
          if (instanceIdStr.indexOf(":")>=0) {
            log("inst="+instanceIdStr);
            parts = instanceIdStr.split(":");
         ...
         warn("AppId: "+ appId+" "+versionStr+" [no id tag]");   */  
      }    
